#!/usr/bin/env bash
# jira-restore-database.sh [backup-file-name]
#
# Replaces the Jira database with one of the dumps the backups service wrote.
#
#   ./jira-restore-database.sh               list and ask
#   ./jira-restore-database.sh <file-name>   restore that one
#
...
DB_NAME="$(env_of JIRA_DB_NAME)"; DB_USER="$(env_of JIRA_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Jira." >&2
  exit 1
fi
