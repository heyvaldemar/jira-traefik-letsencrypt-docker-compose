#!/bin/bash
# End-to-end tests for the jira-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-jira}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-jira-traefik-letsencrypt-docker-compose.yml}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./jira-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./jira-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./jira-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./jira-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
